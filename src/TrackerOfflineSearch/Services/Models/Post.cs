...
public record Post
{
    ...
    public string Url => string.Format(CultureInfo.InvariantCulture, PostUrlTemplate, Id);

    public string TorrentUrl => string.Format(CultureInfo.InvariantCulture, TorrentUrlTemplate, Id);

    public string MagnetUrl
    {
        get
        {
            var tracker = TrackerId == 1
                ? Tracker1Template
                : string.Format(CultureInfo.InvariantCulture, TrackerNTemplate, TrackerId);
            var trackerEncoded = HttpUtility.UrlEncode(tracker);
            var titleEncoded = HttpUtility.UrlEncode(Title);

            return string.Format(MagnetUrlTemplate, Hash, trackerEncoded, titleEncoded);
        }
    }
