...
public class ProgressViewModel : ActivatableViewModel, IWizardPageViewModel
{
    ...
    private async Task<int> ImportDocumentsAsync(IIndexWriterSession writerSession, string archivePath, bool simpleIndex, CancellationToken ct)
    {
        ...
        _progressMessageSubject.OnNext(string.Format(CultureInfo.CurrentUICulture, "Импортировано {0:N0} документов", 0));
        ...
        await Parallel.ForEachAsync(
            ...
            (post, token) =>
            {
                ...
                {
                    lastTime.Restart();
                    _progressMessageSubject.OnNext(string.Format(CultureInfo.CurrentUICulture, "Импортировано {0:N0} документов", count));
                }

                return ValueTask.CompletedTask;
            }).ConfigureAwait(false);

        _progressMessageSubject.OnNext(string.Format(CultureInfo.CurrentUICulture, "Импортировано {0:N0} документов", total));
    ...
    private static readonly string[] _infoTips = [
    ...
    {
        var tipIndex = index % _infoTips.Length;

        return _infoTips[tipIndex];
    }
